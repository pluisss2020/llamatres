...
if ( !isset($_SESSION['username']) && !isset($_SESSION['userid']) ){ //No hay ninguna sesion iniciada
    ...
    if ( $idcnx = mysqli_connect(DB_HOST,DB_USER,DB_PASS,DB_NAME) ){ //la conexion a la base de datos es ok
	   ...
	   if (isset($_POST['login_username'])){     //login
            $sql = 'SELECT nick,passwd,id,rol,email FROM usuarios WHERE (nick="' . $_POST['login_username'].'" OR email="' . $_POST['login_username']. '") && passwd="' . md5($_POST['login_userpass']) . '" LIMIT 1';
            //echo $sql;
			...
			if ( $res = mysqli_query($idcnx,$sql) ){	
                if ( mysqli_num_rows($res) == 1 ){
                    ...
                    $A=Persona::traer_datos($user['id']);
                    $_SESSION['nro_jugador'] = $A['nro_jugador'];
					echo 1;
                    //echo $user['rol'];     
                }
                else
